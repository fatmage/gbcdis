...
let decode_file file_ch =
  let rec decode_aux acc =
    ...
    | Some byte ->
        let next_instruction =
          ...
          | 0x00 -> iNOP
          | 0x01 ->
              let n16 = read_2bytes file_ch in
              iLD_rn16 BC n16
          | 0x02 -> iLD_r16pA BC
          | 0x03 -> iINC_r16 BC
          | 0x04 -> iINC_r8 B
          ...
          | 0xFF -> iRST_vec 0x38
          (* CB-prefixed *)
          | 0xCB -> match_prefixed file_ch
          | _ -> iNOP
        in
        decode_aux (next_instruction :: acc)
  in
  decode_aux []

let label_comp : label -> label -> int =
 fun l1 l2 ->
  let first_comparision = compare (fst l1) (fst l2) in
  match first_comparision with 0 -> compare (snd l1) (snd l2) | n -> n
...
let make_labels instructions =
  let rec label_aux is acc pos =
    match is with
    ...
    | i :: is -> (
        ...
        | Binary (mnemonic, arg1, arg2, length) -> (
            ...
            let target = int_of_string_opt arg2 in
            match (mnemonic, target) with
            | "JP", Some addr ->
                label_aux is
                  ((addr, Printf.sprintf "Address_%s:" arg2) :: acc)
                  next_pos
            | "JR", Some addr ->
                let abs_address = pos + 2 + (addr |> int8_of_int) in
                label_aux is
                  ((abs_address, Printf.sprintf "Address_0x%04X:" abs_address)
                  :: acc)
                  next_pos
            | "CALL", Some addr ->
                label_aux is
        ...
        | Unary (mnemonic, arg, length) -> (
            let next_pos = pos + length in
            let target = int_of_string_opt arg in
            match (mnemonic, target) with
            | "JP", Some addr ->
                label_aux is
                  ((addr, Printf.sprintf "Address_%s:" arg) :: acc)
                  next_pos
            | "JR", Some addr ->
                let abs_address = pos + 2 + (addr |> int8_of_int) in
                label_aux is
...
let process_file file_ch =
  print_endline "Decoding...";
  let is = decode_file file_ch in
  print_endline "Making labels...";
